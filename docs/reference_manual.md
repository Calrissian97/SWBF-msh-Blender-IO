...
- Cloth meshes should follow the same node hierarchy as skinned meshes (see [Skeletons and Skinning](#skeletons-and-skinning)) with their meshes either a child of the Armature and Parent Bone set to their enveloped bone (re-parenting not necessary for imported cloth models) or simply parented to a valid object in the msh scene.
