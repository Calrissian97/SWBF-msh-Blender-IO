...
- Cloth meshes can follow the same node hierarchy as skinned meshes (see [Skeletons and Skinning](#skeletons-and-skinning)) with their meshes either a child of the Armature and Parent Bone set to their enveloped bone (default importer behavior) or simply parented to an object in the msh scene e.g., "dummyroot".
